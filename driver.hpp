...
//K4: Aiuta a gestire il tipo di operazione sulla variabile, che può essere un assegnamento, un' inizializzazione o un binding
enum varHandleType {
  INIT,
  BIND,
  ASSIGN
...
/// variabile globale Classe per la rappresentazione di variabili globali
...
class GlobalVariableAST: public RootAST{
  private:
    ...
    bool isArr;
  public:
    GlobalVariableAST(std::string Name, double Size=-1, bool isArr = false);
    Value* codegen(driver& drv) override;
...
private:
  ...
  bool isArr;
public:
  VariableExprAST(const std::string &Name, ExprAST* Exp = nullptr, bool isArr = false);
  lexval getLexVal() const override;
  Value *codegen(driver& drv) override;
};
...
/// IfExprAST Classe per la rappresentazione di operatori ternari
...
/// VarBindingsAST Classe per la rappresentazione della creazione e assegnazioni di una variabile
...
class VarBindingsAST : public InitAST{
  ...
  public:
    ...
    varHandleType getType() override;
    const std::string& getName() override;
};

/// ArrayBindingAST Classe per la rappresentazione di creazione e assegnazioni di array
...
/// AssigmentExprAST Classe per la rappresentazione di assegnazioni
